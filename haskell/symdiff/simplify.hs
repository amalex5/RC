...
simplify' (Add x (Neg y)) = Sub (simplify' x) (simplify' y) 
simplify' (Add (Neg x) y) = Sub (simplify' y) (simplify' x)
simplify' (Add x y)
...
simplify' (Div (Val x) (Val y))
  | x `mod` y == 0 && y /= 0 = Val ( x `div` y )
simplify' (Div x y)
  | simplify' y == (Val 1) = simplify' x
  | simplify' x == (Val 0) = Val 0
...
-- rebuildSumTree takes the output of bubbleSums
--and turns it back into a valid Expr tree
...
-- it looks at its children, and:
-- a) if the child is a number, it adds it to a running sum
-- b) if the child is an add node, it recurs
-- c, if the child is something else, it adds it to a list of other stuff
-- and returns a tuple whose first element is the running sum
-- and whose second element is a list of all the stuff that couldn't be added
